...
def show3():
   ...
   while True:
       if (pd + ad) == nlw and p2 > rp and percent < rp:
          ...
          Label(root, text = 'wrong input given').grid(row = 8, column = 1)
...
if __name__ == __main__ :  
    ...
    Button(root, text='Show', command=show1).grid(row=2, column=2, sticky=W, pady=4)
    
    Button(root, text='Show', command=show2).grid(row=5, column=2, sticky=W, pady=4)
    
    Button(root, text='Show',command = show3).grid(row=8, column=2,sticky=W, pady=4)
